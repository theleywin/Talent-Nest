    ...
    const renderButton = () => {
        ...
        switch (connectionStatus?.data?.status) {
            ...
            default:
                return (
                    <button
                        className='px-3 py-1 rounded-full text-sm bg-white border border-green-800 text-green-700 hover:bg-green-700 hover:text-white transition-colors duration-200 flex items-center'
        ...
        <div className='flex items-center justify-between mb-4'>
            <Link to={`/profile/${user.username}`} className='flex items-center flex-grow'>
                ...
                <div>
                    ...
                    <p className='text-xs text-green-900'>{user.headline}</p>
                </div>
            </Link>
            {renderButton()}
        </div>
